...
// Holds every raw upstream (German) job-detail field as-is - nothing is
// dropped during the transform, only excluded from the serialized output.
// Both decorators are required together on each field: @Expose so
// plainToInstance keeps it under excludeExtraneousValues, @Exclude
// (toPlainOnly) so instanceToPlain still leaves it out of the JSON output.
// English getters live on the subclass (VacancyDto).
export class VacancyRawDto {
  @Expose()
